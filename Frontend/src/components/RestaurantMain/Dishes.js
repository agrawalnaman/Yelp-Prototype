...
class Dishes extends Component {
    ...
    componentDidMount() {
        ...
        axios.get("http://35.163.78.149:3001/getRestaurantDishes", data).then((response) => {
    ...
    submitEditDish = (e) => {
        ...
        axios
            .post("http://35.163.78.149:3001/restaurantEditNewDish", data)
            .then((response) => {
                console.log("Status Code : ", response.status);
                if (response.status === 200) {
                    window.alert("Dish Edited Successfully");
                    var data1 = { params: { idRestaurants: localStorage.getItem("r_id") } };
                    axios.get("http://35.163.78.149:3001/getRestaurantDishes", data1).then((response) => {
                        //update the state with the response data
                        console.log(response.data);                        
                        this.setState({
                            // dishes: response.data.dishes,
    ...
    submitAddDish = (e) => {
        ...
        // this.props.signup(data);
        axios
            .post("http://35.163.78.149:3001/restaurantAddNewDish", data)
            .then((response) => {
                console.log("Status Code : ", response.status);
                if (response.status === 200) {
                    window.alert("Dish Added To Menu!");
                    var data2 = { params: { idRestaurants: localStorage.getItem("r_id") } };
                    axios.get("http://35.163.78.149:3001/getRestaurantDishes", data2).then((response) => {
                        //update the state with the response data
                        console.log(response.data);
                        this.setState({
                            // dishes: response.data.dishes,
